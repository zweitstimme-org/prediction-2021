...
model_file <- "../model_code/ger_combined_model.stan"
...
structural_inits <- readRDS(paste0("../../data/ger/structural_inits/", Election, "_structural_inits_only_l1.RDS"))
...
election_pred_E <- dat[dat$election==election_id,c(predictors)] %>% matrix(ncol = 1)
rownames(election_pred_E) <- dat[dat$election==election_id,"party"]
election_pred_E <- election_pred_E[party_names,] %>% matrix(ncol = 1)
election_pred_E[,c(1)] <- election_pred_E[,c(1)] / 100
...
results <- stan(file = '../model_code/ger_combined_model.stan', data = forstan, init = initlist,
            iter = nIter, chains = nChains, thin = 1, control = list(adapt_delta = 0.80, max_treedepth = 10) )

saveRDS(results, file = paste0("../../output/ger/draws/combined_model/res_brw_", Election,"_",cutoff,"_only_l1.RDS"))
...
saveRDS(draws_forecast_levels, 
        file=paste0("../../output/ger/draws/combined_model/draws_forcast_levels_",Election,"_",cutoff,"_only_l1.RDS")
)
}
}
