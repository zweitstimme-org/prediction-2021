...
Elections <- c(2017, 2013, 2002, 2005, 2009)
...
results <- stan(file = '../model_code/combined_model.stan', data = forstan, init = initlist,
            iter = nIter, chains = nChains, thin = 1, control = list(adapt_delta = 0.90, max_treedepth = 10) )

results <- stan(file = '../model_code/combined_model1.stan', data = forstan,
                iter = 2500, warmup = 2000, chains = nChains, thin = 1, control = list(adapt_delta = 0.99) )

results <- stan(file = '../model_code/combined_model1.stan', data = forstan,
                iter = nIter, chains = nChains, thin = 1, control = list(adapt_delta = 0.99) )


saveRDS(results, file = paste0("../../output/ger/draws/combined_model/res_brw_", Election,"_",cutoff,".RDS"))
