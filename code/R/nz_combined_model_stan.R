...
model_file <- "../model_code/nz_combined_model.stan"

Elections <- c(2017, 2014, 2011)


for(Election in Elections){

  
structural_inits <- readRDS(paste0("../../data/nz/structural_inits/", Election, "_structural_inits.RDS"))
...
nz_polls <- readRDS("../../data/nz/Polls/nz_all_polls.RDS") 

el <- Election

nz_polls %<>% 
  filter(Election == el) %>% # Filter Polls for Election
  mutate(party = as.character(as_factor(party))) 

nz_polls$support[is.na(nz_polls$support)] <- 0
...
results <- stan(file = model_file, data = forstan, init = initlist,
            iter = nIter, chains = nChains, thin = 1, control = list(adapt_delta = 0.80, max_treedepth = 10) )

saveRDS(results, file = paste0("../../output/nz/draws/combined_model/res_brw_", Election,"_",cutoff,".RDS"))
...
saveRDS(draws_forecast_levels, 
        file=paste0("../../output/nz/draws/combined_model/draws_forcast_levels_",Election,"_",cutoff,".RDS")
)
}
}
